...
{
    // 0) Parse thread count from command line (default = 1)
    int thread_count = 1;
    if (argc > 1) {
        try {
    ...
    { // Set to true to run this section
        ...
        ForexGraph graph = buildForexGraphFromCsvs(currencyFiles, thread_count);
        ...
        auto arbitrage = detectArbitrage(graph, thread_count);
    ...
    TimeSeriesArbitrageDetector timeSeriesDetector(currencyFiles, thread_count);
