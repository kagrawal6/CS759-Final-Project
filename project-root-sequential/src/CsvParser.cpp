...
// Estimate line count based on file size and average line length
static size_t approximateLineCount(size_t fileSize, size_t avgLineLen = 100) {
    return (fileSize + avgLineLen - 1) / avgLineLen; // round up
}

// Helper function to map a file into memory
...
static void unmapFile(char* data, size_t size) {
    if (data && munmap(data, size) == -1) {
        perror("munmap failed");
    }
}

// Read and parse bid/ask CSVs for a currency pair using mmap and reserve optimization
std::vector<CurrencyPairData> readCurrencyPairCsvs(
    const std::string& bidFilepath,
    const std::string& askFilepath,
    const std::string& baseCurrency,
    ...
    // Task 1.3: Reserve capacity based on estimated line count
    size_t approxLines = approximateLineCount(std::min(bidSize, askSize));
    data.reserve(approxLines);
    ...
    while (bidPtr < bidEnd && askPtr < askEnd) {
        ...
        *askLineEnd = '\0';
        ...
        // Verify timestamps match
        if (bidTokens[0] && askTokens[0] && strcmp(bidTokens[0], askTokens[0]) == 0) {
            CurrencyPairData entry;
            ...
            std::cerr << "Timestamp mismatch or parse error at line: "
                      << (bidTokens[0] ? bidTokens[0] : "<null>") << " vs "
                      << (askTokens[0] ? askTokens[0] : "<null>") << "\n";
        }
...
// Build ForexGraph from multiple CSV sources (unchanged)
ForexGraph buildForexGraphFromCsvs(
    const std::vector<std::tuple<std::string, std::string, std::string, std::string>>& currencyFiles) {
    ForexGraph graph;
    for (const auto& [bidFilepath, askFilepath, baseCurrency, quoteCurrency] : currencyFiles) {
